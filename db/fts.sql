...
(
    ...
    LOCKEDBY
        VARCHAR(255),
    CONSTRAINT DATABASECHANGELOGLOCK_PKEY PRIMARY KEY
        (
         ID
            )
);
...
(
    ...
    CONSTRAINT UKM3IOM37EFAXD5EUCMXJQQCBE9 UNIQUE
        (
         PHONE
            )
);
...
(
    ...
    ATTRIBUTE_NAME
        VARCHAR(255),
    CONSTRAINT PRODUCT_ATTRIBUTES_PKEY PRIMARY KEY
        (
         ID
            )
);
...
(
    ...
    PRODUCT_ID
        INTEGER,
    CONSTRAINT
        FKD112RX0ALYCDDSMS029IIFRIH
        FOREIGN
            KEY
            (
             CATEGORY_ID
                ) REFERENCES CATEGORIES,
    CONSTRAINT FKLDA9RAD6S180HA3DL1NCSP8N7 FOREIGN KEY
        (
         PRODUCT_ID
            ) REFERENCES PRODUCTS
);
...
(
    ...
    CONSTRAINT UK525CSMEMMGTOICJCFHCPF3PK0 UNIQUE
        (
         UNIT_NAME
            )
);
...
(
    ...
    STATUS             VARCHAR(255),
    FTS                TSVECTOR,
    CONSTRAINT PRODUCT_VARIANTS_PKEY PRIMARY KEY
        (
         ID
            ),
    CONSTRAINT PRODUCT_VARIANTS_UNIT_ID_FKEY FOREIGN KEY
        (
         UNIT_ID
            ) REFERENCES UNITS,
    CONSTRAINT PRODUCT_VARIANTS_PRODUCT_ID_FKEY FOREIGN KEY
        (
         PRODUCT_ID
            ) REFERENCES PRODUCTS
);
...
(
    ...
    UNIT_ID            INTEGER,
    CONSTRAINT PRODUCT_ATTRIBUTE_VALUES_PKEY PRIMARY KEY
        (
         ID
            ),
    CONSTRAINT PRODUCT_ATTRIBUTE_VALUES_PRODUCT_VARIANT_ID_FKEY FOREIGN KEY
        (
         PRODUCT_VARIANT_ID
            ) REFERENCES PRODUCT_VARIANTS,
    CONSTRAINT PRODUCT_ATTRIBUTE_VALUES_ATTR_ID_FKEY FOREIGN KEY
        (
         ATTR_ID
...
(
    ...
    PATH
                       VARCHAR(255),
    PRODUCT_VARIANT_ID INTEGER,
    CONSTRAINT PRODUCT_IMAGES_PKEY PRIMARY KEY
        (
         ID
            ),
    CONSTRAINT PRODUCT_IMAGES_PRODUCT_VARIANT_ID_FKEY FOREIGN KEY
        (
         PRODUCT_VARIANT_ID
            ) REFERENCES PRODUCT_VARIANTS
);
...
(
    ...
    CONSTRAINT UKDU5V5SR43G5BFNJI4VB8HG5S3 UNIQUE
        (
         PHONE
            )
);
...
(
    ...
    CONSTRAINT ORDERS_USER_ID_FKEY FOREIGN KEY
        (
         USER_ID
            ) REFERENCES USERS
);
...
(
    ...
    PRICE
        BIGINT,
    CONSTRAINT
        FKJYU2QBQT8GNVNO9OE9J2S2LDK
        FOREIGN
            KEY
            (
             ORDER_ID
                ) REFERENCES ORDERS,
    CONSTRAINT FKSX0ENYL805EMARBR0BGOLN1OQ FOREIGN KEY
        (
         PRODUCT_VARIANT_ID
            ) REFERENCES PRODUCT_VARIANTS
);
...
(
    ...
    ORDER_ID           INTEGER,
    CONSTRAINT ORDER_HISTORIES_PKEY PRIMARY KEY
        (
         ID
            ),
    CONSTRAINT ORDER_HISTORIES_ORDER_ID_FKEY FOREIGN KEY
        (
         ORDER_ID
            ) REFERENCES ORDERS
);
...
(
    ...
    CONSTRAINT PAYMENTS_ORDER_ID_FKEY FOREIGN KEY
        (
         ORDER_ID
            ) REFERENCES ORDERS
);
...
CREATE
    OR REPLACE FUNCTION PUBLIC.PRODUCTS_FTS_TRIGGER() RETURNS TRIGGER AS
$$
BEGIN
    NEW.fts
        :=
...
CREATE
    OR REPLACE FUNCTION PUBLIC.PRODUCT_VARIANTS_FTS_TRIGGER() RETURNS TRIGGER AS
$$
BEGIN
    NEW.fts
        :=
...
CREATE
    OR REPLACE FUNCTION UPDATE_CUSTOMERS_FTS() RETURNS TRIGGER AS
$$
BEGIN
    NEW.fts
        := to_tsvector('simple', unaccent(coalesce(NEW.customer_name, '') || ' ' || coalesce(NEW.address, '') || ' ' ||
                                          coalesce(NEW.phone, '')));
    RETURN NEW;
END
$$
...
-- Tạo lại trigger
CREATE
    OR REPLACE TRIGGER TRG_ORDERS_FTS BEFORE INSERT
    OR
