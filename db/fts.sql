...
CREATE TRIGGER TRG_CUSTOMERS_FTS BEFORE INSERT
    OR
...
CREATE TRIGGER TRG_ORDERS_FTS BEFORE INSERT
    OR
...
CREATE TRIGGER TRG_USERS_FTS BEFORE INSERT
    OR
...
ALTER TABLE PRODUCT_VARIANTS ALTER COLUMN created_by DROP NOT NULL;
ALTER TABLE PRODUCT_ATTRIBUTE_VALUES ALTER COLUMN created_by DROP NOT NULL;
ALTER TABLE USERS ALTER COLUMN created_by DROP NOT NULL;
ALTER TABLE ORDERS ALTER COLUMN created_by DROP NOT NULL;
ALTER TABLE ORDER_HISTORIES ALTER COLUMN created_by DROP NOT NULL;
...
SET
    FTS = TO_TSVECTOR(
            'simple',
            UNACCENT (
                    COALESCE(RECEIVER_NAME, '') || ' ' || COALESCE(RECEIVER_ADDRESS, '') || ' ' || COALESCE(RECEIVER_PHONE, '') || ' ' || COALESCE(NOTE, '')
            )
          );
